...
describe('renderResearchCommandContract', () => {
  it('requires /search runs to prefer MCP web search before the research command fallback', () => {
    ...
    expect(prompt).toContain(
      'first use an enabled MCP web-search tool when one is available',
    );
    expect(prompt).toContain(
      'If MCP web search is not available and the OD command fails because Tavily is not configured or unavailable',
    );
    expect(prompt).toContain(
